              ...
              <div class="form-group">
                ...
                <select class="custom-select rounded-0" id="reservation_status" name="reservation_status">
                  @foreach ($reservation_status as $status)
                    <option value="{{ $status->reservation_status_id }}" {{ $reservations['reservation_status_id'] == $status->reservation_status_id ? 'selected' : '' }}>{{ $status->reservation_status_name }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
                ...
                <select class="custom-select rounded-0" id="reservation_type" name="reservation_type">
                  @foreach ($reservation_type as $type)
                    <option value="{{ $type->reservation_type_id }}" {{ $reservations['reservation_type_id'] == $type->reservation_type_id ? 'selected' : '' }}>{{ $type->reservation_type_name }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
                ...
                <select class="custom-select rounded-0" id="room_type" name="room_type">
                  @foreach ($room_type as $room)
                    <option value="{{ $room->room_type_id }}" {{ $reservations['room_type_id'] == $room->room_type_id ? 'selected' : '' }}>{{ $room->room_type_name }}</option>
                  @endforeach
                </select>
              </div>
              <div class="form-group">
                <label for="first_name">First Name</label>
                <input type="text" class="form-control" id="first_name" name="first_name" value="{{ $reservations['first_name'] }}" placeholder="Enter first name">
              </div>
              <div class="form-group">
                <label for="last_name">Last Name</label>
                <input type="text" class="form-control" id="last_name" name="last_name" value="{{ $reservations['last_name'] }}" placeholder="Enter last name">
              </div>
              <div class="form-group">
                <label for="phone_number">Phone Number</label>
                <input type="number" class="form-control" id="phone_number" name="phone_number" value="{{ $reservations['phone_number'] }}" placeholder="Enter phone number">
              </div>
              <div class="form-group">
